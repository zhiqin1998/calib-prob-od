...
def instances_to_json(instances, img_id, cat_mapping_dict=None, unc_ann=False, img_dim=None):
    """
    ...
    if instances.has("pred_boxes_covariance"):
        if unc_ann: # keep as xyxy and scale by image res
            pred_boxes_covariance = instances.pred_boxes_covariance.cpu().tolist()
            for i in range(len(pred_boxes_covariance)):
                for j in range(len(pred_boxes_covariance[i])):
                    pred_boxes_covariance[i][j][j] = pred_boxes_covariance[i][j][j] * (
                        img_dim[0] if j % 2 == 0 else img_dim[1]) ** 2
        else:
