...
# Project Imports
from probabilistic_modeling.modeling_utils import covariance_output_to_cholesky, clamp_log_variance, get_probabilistic_loss_weight, sigmoid_focal_loss_soft_jit
...
class ProbabilisticRetinaNet(RetinaNet):
    ...
    def __init__(self, cfg):
        ...
        # Send to device
        self.to(self.device)
        self.ann_unc = False if 'ANN_UNCERTAINTY' not in cfg else cfg.ANN_UNCERTAINTY
    ...
    def forward(
        ...
        if self.training:
            ...
            gt_classes, gt_boxes, all_matched_indx = self.label_anchors(
                anchors, gt_instances)

            gt_logits, gt_err_bbox, gt_weights = [], [], []
            if len(gt_instances) and self.ann_unc:
                for i, (gt_per_image, matched_idxs) in enumerate(zip(gt_instances, all_matched_indx)):
                    gt_logits_i = gt_per_image.gt_logits[matched_idxs]
                    gt_err_bbox_i = gt_per_image.gt_err_bbox[matched_idxs]

                    num_classes = gt_logits_i.shape[-1] - 1
                    mask = torch.logical_or(gt_classes[i] == (num_classes + 1), gt_classes[i] == -1)
                    bg_count = torch.sum(mask)
                    gt_logits_i[mask] = cat(
                        [gt_logits_i.new_zeros((bg_count, num_classes)), gt_logits_i.new_ones((bg_count, 1))], dim=1)
                    # gt_classes[i] = torch.clamp(gt_classes[i], max=num_classes)
                    gt_weights_i = gt_logits_i.new_ones((gt_logits_i.shape[0],))
                    gt_weights_i[~mask] = 1 - gt_logits_i[~mask][:, -1]

                    gt_logits.append(gt_logits_i)
                    gt_err_bbox.append(gt_err_bbox_i)
                    gt_weights.append(gt_weights_i)
            if not self.ann_unc:
                gt_logits, gt_err_bbox, gt_weights = None, None, None
            ...
            losses = self.losses(
                ...
                pred_anchor_deltas,
                pred_logits_vars,
                pred_anchor_deltas_vars, gt_logits, gt_err_bbox, gt_weights)
    ...
    def label_anchors(self, anchors, gt_instances):
        ...
        for gt_per_image in gt_instances:
            ...
            all_matched_indx.append(matched_idxs)
            ...
            if len(gt_per_image) > 0:
                ...
                gt_labels_i[anchor_labels == 0] = self.num_classes + (1 if self.ann_unc else 0)
                # Anchors with label -1 are ignored.
                gt_labels_i[anchor_labels == -1] = -1
            else:
                matched_gt_boxes_i = torch.zeros_like(anchors.tensor)
                gt_labels_i = torch.zeros_like(matched_idxs) + self.num_classes + (1 if self.ann_unc else 0)

            gt_labels.append(gt_labels_i)
            matched_gt_boxes.append(matched_gt_boxes_i)

        return gt_labels, matched_gt_boxes, all_matched_indx
    ...
    def losses(
            ...
            pred_bbox_cov=None, gt_logits=None, gt_err_bbox=None, gt_weights=None):
        """
        Args:
        ...
        if gt_logits is not None:
            gt_logits = torch.stack(gt_logits)
            gt_err_bbox = torch.stack(gt_err_bbox)
            gt_weights = torch.stack(gt_weights)

        valid_mask = gt_labels >= 0
        pos_mask = (gt_labels >= 0) & (gt_labels < (self.num_classes + (1 if self.ann_unc else 0)))
        ...
        if gt_logits is None:
            ...
            gt_classes_target = gt_logits[valid_mask][..., :-1].to(pred_class_logits[0].dtype)
        ...
        if self.compute_cls_var:
            ...
            if self.ann_unc:
                loss_cls = sigmoid_focal_loss_soft_jit(
                ...
                # loss_cls = (loss_cls * gt_weights[valid_mask][..., None]).sum() / max(1, self.loss_normalizer)
        ...
        if self.compute_bbox_cov:
            ...
            elif self.bbox_cov_loss == 'dmmv2':
                loss_box_reg = smooth_l1_loss(
                    pred_anchor_deltas,
                    gt_anchors_deltas,
                    beta=self.smooth_l1_beta)

                alpha = torch.clamp(gt_logits[pos_mask][..., -1:], min=0.01)  # last idx is bg, alpha = 1 - target_obj, target_obj = 1 - bg_prob
                z = torch.distributions.Normal(0, 1).icdf(1 - alpha / 2)
                target_var = (gt_err_bbox[pos_mask] / z) ** 2

                if self.bbox_cov_type == 'diagonal':
                    # Compute second moment matching term.
                    second_moment_matching_term = smooth_l1_loss(
                        torch.exp(pred_bbox_cov), target_var, beta=self.smooth_l1_beta).masked_fill_(target_var == 0, 0)
                    loss_box_reg += second_moment_matching_term
                    # loss_box_reg *= gt_weights[pos_mask][..., None]
                    loss_box_reg = 0.01 * torch.sum(
                        loss_box_reg) / (max(1, self.loss_normalizer) * 4)
                else:
                    raise NotImplementedError('Cannot compute interval for multivariate covariance')
            else:
                raise ValueError(
            ...
            if self.compute_bbox_cov and self.bbox_cov_loss == 'dmmv2':
                standard_regression_loss = smooth_l1_loss(
                    pred_anchor_deltas,
                    gt_anchors_deltas,
                    beta=self.smooth_l1_beta,
                )
                standard_regression_loss *= gt_weights[pos_mask][..., None]
                standard_regression_loss = torch.sum(standard_regression_loss) / max(1, self.loss_normalizer)
            else:
                standard_regression_loss = smooth_l1_loss(
                    pred_anchor_deltas,
                    gt_anchors_deltas,
...
class ProbabilisticRetinaNetHead(RetinaNetHead):
    ...
    def forward(self, features):
        ...
        for feature in features:
            ...
            box_out = self.bbox_subnet(feature)
            logits.append(self.cls_score(cls_out))
            bbox_reg.append(self.bbox_pred(box_out))
            if self.compute_cls_var:
                logits_var.append(self.cls_var(cls_out))
            if self.compute_bbox_cov:
                bbox_cov.append(self.bbox_cov(box_out))
