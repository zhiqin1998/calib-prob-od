...
from probabilistic_modeling.modeling_utils import get_probabilistic_loss_weight, clamp_log_variance, covariance_output_to_cholesky, cross_entropy_soft_targets
...
class ProbabilisticROIHeads(StandardROIHeads):
    ...
    def forward(
        ...
        if self.training and not self.is_mc_dropout_inference:
            ...
            if self.ann_unc:
                for p in proposals:
                    num_classes = p.gt_logits.shape[-1] - 1
                    mask = p.gt_classes == (num_classes + 1)
                    bg_count = torch.sum(mask)
                    p.gt_logits[mask] = cat(
                        [p.gt_logits.new_zeros((bg_count, num_classes)), p.gt_logits.new_ones((bg_count, 1))], dim=1)
                    # p.gt_classes[mask] = num_classes
                    p.weights = p.gt_logits.new_ones((p.gt_logits.shape[0],))
                    p.weights[~mask] = 1 - p.gt_logits[~mask][:, -1]
                    # set bg unc class with secondary class, should be fine since we nolonger use it for loss
                    mask = p.gt_classes == num_classes
                    p.gt_classes[mask] = p.gt_logits[mask][..., :-1].argmax(-1)
        del targets
    ...
    def _sample_proposals(
        ...
        if has_gt:
            ...
            gt_classes[matched_labels == 0] = self.num_classes + (1 if self.ann_unc else 0)
            # Label ignore proposals (-1 label)
            gt_classes[matched_labels == -1] = -1
        else:
            gt_classes = torch.zeros_like(matched_idxs) + self.num_classes + (1 if self.ann_unc else 0)

        sampled_fg_idxs, sampled_bg_idxs = subsample_labels(
            gt_classes, self.batch_size_per_image, self.positive_fraction, self.num_classes + (1 if self.ann_unc else 0)
        )
...
class ProbabilisticFastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        num_bbox_reg_classes = 1.0 if cls_agnostic_bbox_reg else (num_classes) #+ (1 if bbox_cov_loss == 'dmmv2' else 0))
    ...
    def losses(self, predictions, proposals, current_step=0):
        ...
        if len(proposals):
            ...
            if proposals[0].has("gt_raw_bbox"):
                ann_unc = True
                assert proposals[0].has("gt_logits")
                gt_logits = cat([p.gt_logits for p in proposals], dim=0)
                gt_err_bbox = cat([p.gt_err_bbox for p in proposals], dim=0)
                gt_weights = cat([p.weights for p in proposals], dim=0)
        ...
        if no_instances:
            ...
            if self.compute_cls_var:
                ...
                if ann_unc:
                    loss_cls = cross_entropy_soft_targets(pred_class_logits, gt_logits, reduction='mean')
                    # loss_cls = (cross_entropy_soft_targets(pred_class_logits, gt_logits, reduction='none') * gt_weights).mean()
                else:
                    loss_cls = F.cross_entropy(
        ...
        if no_instances:
            ...
            bg_class_ind = pred_class_logits.shape[1] - 1#(0 if ann_unc else 1)
            ...
            if self.compute_bbox_cov:
                ...
                if self.bbox_cov_loss == 'negative_log_likelihood':
                    ...
                    # Compute to learn annotator uncertainty
                    loss_box_reg = smooth_l1_loss(pred_proposal_deltas,
                                                  gt_proposals_delta,
                                                  self.smooth_l1_beta)

                    alpha = torch.clamp(gt_logits[fg_inds, -1:], min=0.01)  # last idx is bg, alpha = 1 - target_obj, target_obj = 1 - bg_prob
                    z = torch.distributions.Normal(0, 1).icdf(1 - alpha / 2)
                    target_var = (gt_err_bbox[fg_inds] / z) ** 2
                    if self.bbox_cov_type == 'diagonal':
                        # Handel diagonal case
                        second_moment_matching_term = smooth_l1_loss(
                            torch.exp(pred_proposal_covs), target_var, beta=self.smooth_l1_beta).masked_fill_(target_var == 0, 0)
                        loss_box_reg += second_moment_matching_term
                        loss_box_reg *= gt_weights[fg_inds, None]
                        loss_box_reg = 0.01 * torch.sum(
                            loss_box_reg) / (loss_reg_normalizer * 4)
                    else:
                ...
                if self.compute_bbox_cov and self.bbox_cov_loss == 'dmmv2':
                    standard_regression_loss = smooth_l1_loss(pred_proposal_deltas,
                                                              gt_proposals_delta,
                                                              self.smooth_l1_beta,)
                    standard_regression_loss *= gt_weights[fg_inds, None]
                    standard_regression_loss = torch.sum(standard_regression_loss)
                else:
                    standard_regression_loss = smooth_l1_loss(pred_proposal_deltas,
                                                              gt_proposals_delta,
                                                              self.smooth_l1_beta,
